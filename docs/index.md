...
### Prerequisites
A local Docker installation is required to run minikube. Helm is also required.
...
Keep in mind, that the bootstrap script creates a `kubectl` alias which points to `minikube kubectl` so this might conflict with other `kubectl` installations.
